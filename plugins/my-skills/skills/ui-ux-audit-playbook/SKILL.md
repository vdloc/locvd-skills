...
**Sources**: Refactoring UI [RUI] · Practical UI 2e [PUI] · Designing Interfaces 3e [TID] · Web Style Guide 4e [WSG] · How to Design Better UI Components 3.0 [KUL] | **Topic chapters**: 15 | **Generated**: 2026-09-14
...
2. **Collect evidence** — live/dev-server target: follow [evidence.md](evidence.md): Playwright MCP screenshots (375/768/1280px, both modes), accessibility snapshot, focus-order pass, metrics `browser_evaluate` script (font sizes, contrast fails, small targets, off-grid spacing), zoom/reflow check, optional axe injection. Static screenshots/code only: greyscale + squint/blur test, manual measurement from DOM/CSS. Never invent measurements — mark "needs verification" when can't measure.
